...
router.get('/', validate(getArticlesSchema), async (req, res) => {
    let query = {draft: false} as object;
    if (req.query.hasTag) {
        query = {...query, tags: req.query.hasTag}
    }
...
router.get('/all', accessGuard, validate(getArticlesSchema), async (req, res) => {
    let query = {};
    if (req.query.hasTag) {
...
router.patch('/:id', accessGuard, validate(patchArticleById), async (req, res) => {
